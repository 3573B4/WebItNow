...
{
    public partial class fwGM_Configuracion_Proyect : System.Web.UI.Page
    {
        ...
        protected void GetTipoEstatus()
        {
            try
            {
                ...
                string strQuery = "SELECT a.IdEstatus, a.Descripcion, b.Descripcion As IdCliente, c.Descripcion As IdProyecto " +
                                  "  FROM ITM_76 as a, ITM_67 as b, ITM_26 as c " +
        ...
        protected void GetSeccion_1()
        {
            try
            {
                ...
                string strQuery = " WITH CTE AS ( SELECT  ITM_21.Descripcion AS Descripcion_21, " +
                                  "  ROW_NUMBER() OVER (ORDER BY ITM_25.IdPosicion) AS RowNumber " +
                                  " FROM ITM_21 " +
                                  "INNER JOIN ITM_25 ON ITM_21.IdDocumento = ITM_25.IdDocumento " +
                                  "WHERE ITM_21.IdStatus = 1 AND ITM_21.IdTpoAsunto = 0 " +
                                  "  AND ITM_25.IdProyecto = " + Variables.wIdProyecto + " AND ITM_25.IdSeccion = 1 " +
                                  "  AND ITM_25.IdTpoAsunto = " + IdTpoAsunto + " AND ITM_25.IdCliente = '" + IdCliente + "' )," +
                                  ...
                                  "  AND IdCliente = '" + IdCliente + "' ) " +
                                  "SELECT COALESCE(CTE1.Descripcion_21, '') AS Columna1, " +
                                  "       COALESCE(D1.bSeleccion, 0) AS ChBoxSeccion_1_1, " +
                                  "       COALESCE(CTE2.Descripcion_21, '') AS Columna2, " +
                                  "       COALESCE(D2.bSeleccion, 0) AS ChBoxSeccion_1_2, " +
                                  "       COALESCE(CTE3.Descripcion_21, '') AS Columna3, " +
        ...
        protected void GetSeccion_2()
        {
            try
            {
                ...
                string strQuery = " WITH CTE AS ( SELECT  ITM_22.Descripcion AS Descripcion_22, " +
                                  "  ROW_NUMBER() OVER (ORDER BY ITM_25.IdPosicion) AS RowNumber " +
                                  " FROM ITM_22 " +
                                  "INNER JOIN ITM_25 ON ITM_22.IdDocumento = ITM_25.IdDocumento " +
                                  "WHERE ITM_22.IdStatus = 1 AND ITM_22.IdTpoAsunto = 0 " +
                                  "  AND ITM_25.IdProyecto = " + Variables.wIdProyecto + " AND ITM_25.IdSeccion = 2 " +
                                  "  AND ITM_25.IdTpoAsunto = " + IdTpoAsunto + " AND ITM_25.IdCliente = '" + IdCliente + "' )," +
                                  "      Documentos AS ( SELECT IdDocumento, bSeleccion, " +
                                  "  ROW_NUMBER() OVER (ORDER BY IdPosicion) AS RowNumber " +
                                  " FROM ITM_25 " +
        ...
        protected void GetSeccion_3()
        {
            try
            {
                ...
                string strQuery = " WITH CTE AS ( SELECT  ITM_23.Descripcion AS Descripcion_23, " +
                                  "  ROW_NUMBER() OVER (ORDER BY ITM_25.IdPosicion) AS RowNumber " +
                                  " FROM ITM_23 " +
                                  "INNER JOIN ITM_25 ON ITM_23.IdDocumento = ITM_25.IdDocumento " +
                                  "WHERE ITM_23.IdStatus = 1 AND ITM_23.IdTpoAsunto = 0 " +
                                  "  AND ITM_25.IdProyecto = " + Variables.wIdProyecto + " AND ITM_25.IdSeccion = 3 " +
                                  "  AND ITM_25.IdTpoAsunto = " + IdTpoAsunto + " AND ITM_25.IdCliente = '" + IdCliente + "' )," +
                                  "      Documentos AS ( SELECT IdDocumento, bSeleccion, " +
                                  "  ROW_NUMBER() OVER (ORDER BY IdPosicion) AS RowNumber " +
                                  " FROM ITM_25 " +
                                  ...
                                  "  AND IdCliente = '" + IdCliente + "' ) " +
                                  "SELECT COALESCE(CTE1.Descripcion_23, '') AS Columna1, " +
                                  "       COALESCE(D1.bSeleccion, 0) AS ChBoxSeccion_3_1, " +
                                  "       COALESCE(CTE2.Descripcion_23, '') AS Columna2, " +
                                  "       COALESCE(D2.bSeleccion, 0) AS ChBoxSeccion_3_2, " +
                                  "       COALESCE(CTE3.Descripcion_23, '') AS Columna3, " +
        ...
        protected void GetSeccion_4()
        {
            try
            {
                ...
                string strQuery = " WITH CTE AS ( SELECT  ITM_84.Descripcion AS Descripcion_84, " +
                                  "  ROW_NUMBER() OVER (ORDER BY ITM_25.IdPosicion) AS RowNumber " +
                                  " FROM ITM_84 " +
                                  "INNER JOIN ITM_25 ON ITM_84.IdDocumento = ITM_25.IdDocumento " +
                                  "WHERE ITM_84.IdStatus = 1 AND ITM_84.IdTpoAsunto = 0 " +
                                  "  AND ITM_25.IdProyecto = " + Variables.wIdProyecto + " AND ITM_25.IdSeccion = 4 " +
                                  "  AND ITM_25.IdTpoAsunto = " + IdTpoAsunto + " AND ITM_25.IdCliente = '" + IdCliente + "' )," +
                                  "      Documentos AS ( SELECT IdDocumento, bSeleccion, " +
                                  "  ROW_NUMBER() OVER (ORDER BY IdPosicion) AS RowNumber " +
                                  " FROM ITM_25 " +
        ...
        protected void GetSeccion_5()
        {
            try
            {
                ...
                string strQuery = " WITH CTE AS ( SELECT  ITM_24.Descripcion AS Descripcion_24, " +
                                  "  ROW_NUMBER() OVER (ORDER BY ITM_25.IdPosicion) AS RowNumber " +
                                  " FROM ITM_24 " +
                                  "INNER JOIN ITM_25 ON ITM_24.IdDocumento = ITM_25.IdDocumento " +
                                  "WHERE ITM_24.IdStatus = 1 AND ITM_24.IdTpoAsunto = 0 " +
                                  "  AND ITM_25.IdProyecto = " + Variables.wIdProyecto + " AND ITM_25.IdSeccion = 5 " +
                                  "  AND ITM_25.IdTpoAsunto = " + IdTpoAsunto + " AND ITM_25.IdCliente = '" + IdCliente + "' )," +
                                  "      Documentos AS ( SELECT IdDocumento, bSeleccion, " +
                                  "  ROW_NUMBER() OVER (ORDER BY IdPosicion) AS RowNumber " +
                                  " FROM ITM_25 " +
        ...
        protected void GetDatosSeccion(GridView gridView, string checkBoxID, int IdSeccion)
        {
            try
            {
                ...
                // obtener datos de la tabla ITM_25. para la Seccion : Asegurados a los que aplica(n) este/os Documentos(s).
                string strQuery = "SELECT IdPosicion, IdDocumento, bSeleccion" +
                                  "  FROM ITM_25 WHERE IdProyecto = " + IdProyecto + " " +
                                  "   AND IdCliente = '" + IdCliente + "' " +
        ...
        // Método para actualizar los datos en la tabla ITM_25
        private void ActualizarDatosEnTabla(int idProyecto, string idCliente, int idSeccion, string idTpoAsunto, bool[] valoresDocs)
        {
            try
            {
                ...
                // Inicializar datos tabla ITM_25

                string strQuery = $@"UPDATE ITM_25 
                                            SET bSeleccion = 0 
        ...
        protected void Add_tbCarpeta()
        {
            try
            {
                ...
                string IdTpoEvento = (string)Session["IdTpoAsunto"];
                ...
                if (TxtNomProyecto.Text != "NINGUNO")
                {
                    ...
                    iConsecutivo = obtenerConsecutivo("fwAlta_Asunto_GM");
                ...
                string sNomAjustador = (string)Session["NomAsegurado"];
                ...
                // Insertar registro tabla (ITM_26)
                string strQuery = "INSERT INTO ITM_26 (IdProyecto, Referencia, SubReferencia, Descripcion, IdCliente, IdTpoAsunto, Descripcion_Proyecto, Gerente_Responsable, IdTpoGestion, NumPoliza, " +
                                  " NomAsegurado, TpoAsegurado, FecIni_Vigencia, FecFin_Vigencia, Id_Usuario, IdStatus) " +
                                  " VALUES (" + iProyecto + ", '" + sReferencia + "', 0, '" + TxtNomProyecto.Text.Trim() + "', '" + sIdCliente + "', '" + IdTpoEvento + "', " +
                                  "'" + sDescProyecto + "', '" + iGerente + "', " + iTpoGestion + ", '" + sNumPoliza + "', " +
                                  "'" + sNomAjustador + "', " + iTpoAsegurado + ", '" + sIniVigencia + "', '" + sFinVigencia + "', '" + sUsuario + "', 1);" + "\n \n";
                ...
                strQuery += "INSERT INTO ITM_73 (Referencia, SubReferencia, NumPoliza, IdSeguros, IdTpoEvento, IdProyecto, IdRegimen, Fecha_Asignacion, " +
                            " NomAjustador, IdConclusion, IdTpoProyecto, Id_Usuario, IdStatus) " +
                            "  VALUES ('" + sReferencia + "', 0, '" + sNumPoliza + "', '" + sIdCliente + "', '" + IdTpoEvento + "', " +
                            " " + iProyecto + ", 1, '" + fechaActualFormateada + "', '" + sNomAjustador + "', 4, 1, '" + sUsuario + "', 1)";
        ...
        protected void Renumerar_ITM_86()
        {
            try
            {
                ...
                strQuery = "  WITH Renumbered AS ( SELECT IdPosicion, " +
                           ...
                           "  FROM ITM_25 " +
                           "  JOIN Renumbered ON ITM_25.IdPosicion = Renumbered.IdPosicion " +
        ...
        protected void Del_tbCarpeta(int iIdDocumento)
        {
            try
            {
                ...
                string strQuery = "DELETE FROM ITM_25 " +
                                  " WHERE IdStatus = 1 " +
                                  "   AND IdProyecto = " + IdProyecto + " " +
                                  "   AND IdCliente = '" + IdCliente + "' " +
                ...
                strQuery += "  WITH Renumbered AS ( " +
                            ...
                            "   AND IdSeccion = " + Variables.wSeccion + " " +
                            "   AND IdTpoAsunto = " + IdTpoAsunto + " ) " +
                            "UPDATE ITM_25 SET IdPosicion = ( SELECT Renumbered.NewId FROM Renumbered WHERE Renumbered.IdPosicion = ITM_25.IdPosicion ) " +
                            "WHERE ITM_25.IdProyecto = " + IdProyecto + " " +
                            "  AND ITM_25.IdCliente = '" + IdCliente + "' " +
                            "  AND ITM_25.IdSeccion = " + Variables.wSeccion + " " +
                            "  AND ITM_25.IdTpoAsunto = " + IdTpoAsunto + "; ";
                ...
                foreach (GridViewRow row in grdPnlBusqProceso.Rows)
                {
                    if (row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        if (chkbox.Checked)
                        {
                            ...
                            strQuery = "INSERT INTO ITM_25 (IdPosicion, IdProyecto, IdCliente, IdSeccion, IdTpoAsunto, IdDocumento, bSeleccion, IdStatus) " +
                                       "SELECT @IdPosicion, @IdProyecto, @IdCliente, @IdSeccion, @IdTpoAsunto, @IdDocumento, 0, 1 " +
                                       // "  FROM DUAL " +
                                       " WHERE NOT EXISTS ( " +
                                       "SELECT 1 FROM ITM_25 " +
                                       " WHERE IdProyecto = @IdProyecto AND IdCliente = @IdCliente " +
        ...
        protected void BtnRegresar_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("fwGM_Alta_Proyecto.aspx", true);
            return;
        }
